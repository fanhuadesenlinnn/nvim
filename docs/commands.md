...
| 命令 | 什么时候用 | 说明 |
| --- | --- | --- |
| `:BufferLineCyclePrev` | 想切到上一个已打开文件时 | 和快捷键 `<S-h>` 类似。 |
| `:BufferLineCycleNext` | 想切到下一个已打开文件时 | 和快捷键 `<S-l>` 类似。 |
| `:BufferLinePick` | 已打开文件很多，想按字母选择一个时 | 和快捷键 `<Space>bb` 类似。 |
| `:BufferLinePickClose` | 想按字母选择并关闭一个文件时 | 和快捷键 `<Space>bc` 类似。 |
| `:BufferLineTogglePin` | 想固定当前文件，避免被批量关闭时 | 和快捷键 `<Space>bp` 类似。 |
| `:BufferLineCloseOthers` | 想只保留当前文件时 | 和快捷键 `<Space>bo` 类似。 |
| `:BufferLineCloseLeft` | 想关闭左侧所有文件时 | 和快捷键 `<Space>bl` 类似。 |
| `:BufferLineCloseRight` | 想关闭右侧所有文件时 | 和快捷键 `<Space>br` 类似。 |
